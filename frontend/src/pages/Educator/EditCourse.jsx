  ...
  return (
    <div className='max-w-5xl mx-auto p-6 mt-10 bg-white rounded-lg shadow-md'>
      ...
      <div className='flex items-center justify-center gap-[20px] md:justify-between flex-col md:flex-row mb-6 relative'>
        ...
        <h2 className='text-2xl font-semibold md:pl-[60px]'>Add Detail Information regarding courses
        </h2>
        <div className='space-x-2 space-y-2'>
          <button className='bg-black text-white px-4 py-2 rounded-md cursor-pointer' onClick={() => navigate(`/createlecture/${selectedCourse?._id}`)}>Go to lecture page</button>
        </div>
      </div>
      ...
      <div className='bg-gray-50 p-6 rounded-md'>
        ...
        <form className='space-y-6' onSubmit={(e) => e.preventDefault()}>
          ...
          <div>
            ...
            <input type="text" id='title' className='w-full border rounded-md px-4 py-2' placeholder='Course Title' onChange={(e) => setTitle(e.target.value)} value={title} />
          </div>

          <div>
            <label htmlFor="subtitle" className='block text-sm font-medium text-gray-700 mb-1'>Subtitle</label>
            <input type="text" id='subtitle' className='w-full border rounded-md px-4 py-2' placeholder='Course Subtitle' onChange={(e) => setSubtitle(e.target.value)} value={subtitle} />
          </div>

          <div>
            <label htmlFor="description" className='block text-sm font-medium text-gray-700 mb-1'>Description</label>
            <textarea type="text" id='description' className='w-full border rounded-md px-4 py-2 h-24 resize-none' placeholder='Course Description' onChange={(e) => setDescription(e.target.value)} value={description}></textarea>
          </div>
          ...
          <div className='relative w-[300px] h-[170px]'>
            ...
            <div className='flex items-center justify-start gap-[15px]'>
              ...
              <button className='bg-black text-white px-7 py-2 rounded-md hover:bg-gray-500 cursor-pointer' onClick={handleEditCourse} disabled={loading}>{loading ? "Saving..." : "Save"}</button>
            </div>
