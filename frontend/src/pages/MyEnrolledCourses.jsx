    ...
    <div className='min-h-screen w-full px-4 py-9 bg-gray-50'>
...

      {
        userData?.enrolledCourses?.length === 0 ? (
          <p className='text-center w-full text-gray-500'>You have not enrolled in any courses yet</p>
        ) : (
            ...
            {userData?.enrolledCourses?.map((course,index) => (
              <div key={index} className='bg-white rounded-2xl shadow-md overflow-hidden border'>
                ...
                <div className='p-4'>
                  ...
                  <h1 className='px-[10px] text-center py-[10px] border-2 bg-black border-black text-white rounded-[10px] text-[15px] font-light flex items-center jutify-center gap-2 cursor-pointer mt-[10px] hover:bg-gray-600' onClick={() => navigate(`/viewlecture/${course._id}`)}>Watch Now</h1>
                </div>
              </div>
            ))}
          </div>
